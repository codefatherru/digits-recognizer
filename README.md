...
`Neuron_training.py`- скрипт, в котором происходит обучение сети.
в dataset/mnist_train.csv лежит плоская таблица из строк "Y число" + 784 значений 0-255 (фон чёрный, код = 0).
Обученные веса csv формата хранятся в папке "weights".
...
принимает на вход ч\б изображние 168х168, инвертирует (фон - черный с кодом 0) в 28 на 28 и далее в вектор 784.
